...
namespace Business.Concrete
{
    public class AuthManager : IAuthService
    {
        ...
        public IDataResult<User> Login(UserForLoginDto userForLoginDto)
        {
            ...
            if (userToCheck == null) return new ErrorDataResult<User>(Messages.UserNotFoundError);

            if (!HashingHelper.VerifyPasswordHash(userForLoginDto.Password, userToCheck.PasswordHash,
                userToCheck.PasswordSalt)) return new ErrorDataResult<User>(Messages.UserPasswordError);
        ...
        public IResult UserExists(string email)
        {
            ...
            if (userResult.Data != null) return new ErrorResult(Messages.UserEmailAlreadyExists);
